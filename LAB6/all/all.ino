...
const int B = 4;
const int C = 5;
const int D = 6; const int E = 7;
const int F = 9;
const int G = 8; bool Bpress = false;
int UpPin = 10;
int CheckPin = 11;
int num = 0;
int ButtonUp = 0;
...
double previous = 90; long previousTime;
long lastime;
void setup() {
  Serial.begin(115200);
  Wire.begin();
...
void SensorFusion()
{
  ...
  Roll_angle += ((gx - gx_offset) / gyro_factor) * dt; Yaw_angle += (-(gz - gz_offset) / gyro_factor) * dt;
  // Calculate the angle using a Complimentary filter
  Pitch_angle = 0.9 * Pitch_angle + 0.1 * Pitch_accel; Roll_angle = 0.9 * Roll_angle + 0.1 * Roll_accel;
  Serial.print("Yaw = ");
